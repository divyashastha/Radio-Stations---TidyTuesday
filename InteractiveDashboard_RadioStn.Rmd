...
p <- state_stn2 %>%
  ...
  # geom_col_interactive(fill = '#4682B4', color = '#4682B4') + 
...
renderggiraph({
  ...
  ggtitle( "Radio Stations in the United States" ) + #Add map title
  theme(
    legend.position = c(0.5, 0.9), #Choose legend positioning (horizontal, vertical)
    text = element_text(color = "#545454", face="bold"), #This is where we customize the legend text
    plot.background = element_rect(fill = "#fcfdff", color = NA), #Choose the colour of the background behind the title
    panel.background = element_rect(fill = "#fcfdff", color = NA), #This is the main background
    legend.background = element_rect(fill = "#fcfdff", color = NA), #This is the legend background 
    plot.title = element_text(size=18, hjust=0.5, color = "#545454", face="bold"), #This we where we customize the title 
  ) 
